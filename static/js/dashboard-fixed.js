...
document.addEventListener('DOMContentLoaded', function() {
    ...
    firebase.auth().onAuthStateChanged(function(user) {
        if (user) {
            ...
            // User is not logged in, but we'll still load the dashboard with demo data
...
// Load real fields from database (when authenticated)
function loadFields(userId) {
    const fieldCards = document.getElementById('fieldCards');
    fieldCards.innerHTML = '<div class="text-center py-3"><div class="spinner-border text-primary"></div><p class="mt-2">Loading your fields...</p></div>';
    
    // Try to fetch fields from the server
    fetch('/api/fields')
        .then(response => {
            if (!response.ok) {
...
// Load demo fields (for guests or when API fails)
function loadDemoFields() {
    const fieldCards = document.getElementById('fieldCards');
    fieldCards.innerHTML = '';
    
    // Mock fields data for demonstration
    const fields = [
...
function loadWeatherData(fieldId) {
    ...
    fetch(`/api/weather/${fieldId}`)
        ...
        .catch(error => {
            ...
            // Create a dummy forecast for demonstration
            const dummyForecast = [
                { time: new Date().toISOString(), temp: 25, icon: '01d' },
                { time: new Date(Date.now() + 3600000).toISOString(), temp: 26, icon: '02d' },
                { time: new Date(Date.now() + 7200000).toISOString(), temp: 24, icon: '03d' },
                { time: new Date(Date.now() + 10800000).toISOString(), temp: 22, icon: '10d' },
                { time: new Date(Date.now() + 14400000).toISOString(), temp: 21, icon: '10d' },
                { time: new Date(Date.now() + 18000000).toISOString(), temp: 20, icon: '04d' },
                { time: new Date(Date.now() + 21600000).toISOString(), temp: 19, icon: '04d' },
                { time: new Date(Date.now() + 25200000).toISOString(), temp: 18, icon: '01n' }
            ];
            
            // Still create a chart with dummy data
            createWeatherChart(dummyForecast);
        });
}
...
function loadSensorData(fieldId) {
    ...
    fetch(`/api/sensors/${fieldId}`)
        ...
        .catch(error => {
            ...
            // Show mock sensor data for demo
            const mockSensorData = [
                { name: 'Soil Moisture', value: 35, unit: '%', timestamp: new Date().toISOString(), status: 'normal' },
                { name: 'Soil Temperature', value: 22, unit: '°C', timestamp: new Date().toISOString(), status: 'normal' },
                { name: 'Soil pH', value: 6.5, unit: 'pH', timestamp: new Date().toISOString(), status: 'normal' },
                { name: 'Humidity', value: 65, unit: '%', timestamp: new Date().toISOString(), status: 'warning' }
            ];
...
function loadDiseaseMap(fieldId) {
    ...
    fetch(`/api/diseases/${fieldId}`)
        ...
        .catch(error => {
            ...
            // Create a simple placeholder map with OpenStreetMap
            try {
                mapContainer.innerHTML = '';
                ...
                // Add a single marker for demonstration
                map.on('load', function() {
                    // Create marker element
                    const el = document.createElement('div');
                    el.style.width = '20px';
                ...
                mapContainer.innerHTML = '<div class="alert alert-info">Map view is not available. Please try again later.</div>';
            }
        });
}
